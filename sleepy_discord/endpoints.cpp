...
namespace SleepyDiscord {
	...
	ObjectResponse<Message> BaseDiscordClient::uploadFile(Snowflake<Channel> channelID, Buffer buffer, std::string message, std::string filename, Embed embed, RequestSettings<ObjectResponse<Message>> settings) {
		return ObjectResponse<Message>{
			request(Post, path("channels/{channel.id}/messages", { channelID }), settings, "", {
				{ filename, buffer },
				{ "payload_json", createMessageBody(message, embed, TTS::DisableTTS) }
